...
def generate_ip_list(ip_input):
    """Generates a list of IP addresses from one or multiple subnets/IP ranges."""
    ip_list = []
    
    # Split input into multiple IP definitions if separated by commas
    ip_inputs = [ip.strip() for ip in ip_input.split(",")]

    for ip_range in ip_inputs:
        try:
            if "/" in ip_range:  # Subnet notation
                network = ipaddress.IPv4Network(ip_range, strict=False)
                ip_list.extend([str(ip) for ip in network.hosts()])  # Excludes network & broadcast addresses
                ...
                ip_list.extend([str(ipaddress.IPv4Address(ip)) for ip in range(int(start), int(end) + 1)])
            else:
                raise ValueError(f"Invalid input format: {ip_range}. Use CIDR (192.168.1.0/24) or range (192.168.1.1 - 192.168.1.100).")
...
if __name__ == "__main__":
    ip_input = input("Enter one or multiple IP ranges/subnets (comma-separated): ")
    ip_list = generate_ip_list(ip_input)
    process_devices(ip_list, max_workers=10)