...
def check_ssh_availability(ip, username, password, timeout=2):
    """ 
    Checks if SSH is open and authentication works before running any commands.
    Quickly moves on if authentication fails.
    """
...
def process_devices(ip_list, username, password):
    """ 
    Iterates through devices, quickly skipping unknown credentials.
    """
    inventory = {}
    for ip in ip_list:
        print(f"🔍 Checking {ip}...")
        if not check_ssh_availability(ip, username, password):
            continue  # Skip immediately on failed SSH auth
