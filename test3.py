...
def check_ssh_availability(ip, username, password, timeout=2):
    ...
    try:
        ...
        ssh.close()
        return f"🔍 {ip} - SSH Available"
    except paramiko.AuthenticationException:
        logging.info(f"{ip} - Authentication failed, skipping.")
        return f"❌ {ip} - Authentication failed. Skipping..."
    except (paramiko.SSHException, paramiko.ChannelException):
        return f"⚠️ {ip} - SSH not responding"
    except Exception:
        return f"🚫 {ip} - Unreachable or unknown error"
...
def process_device(ip, username, password):
    ...
    ssh_status = check_ssh_availability(ip, username, password)
    print(ssh_status)  # Print SSH status for real-time output

    if "SSH Available" not in ssh_status:
        return None, ssh_status  # Skip non-SSH devices

    device_info = get_juniper_device_info(ip, username, password)
    return device_info, f"✅ {ip} - Juniper device found" if device_info else f"❌ {ip} - Not a Juniper device"
...
def process_devices(ip_list, username, password, max_workers=10):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(future_to_ip):
            ...
            try:
                device_info, status_message = future.result()
                print(status_message)  # Print real-time processing updates
